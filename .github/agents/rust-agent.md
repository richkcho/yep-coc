...
You should use the fetch_webpage tool to recursively gather all information from URL's provided to  you by the user, as well as any links you find in the content of those pages.
...
> In Rust: use `reqwest`, `ureq`, or `surf` for HTTP requests. Use `async`/`await` with `tokio` or `smol` for async I/O. Always handle `Result` and use strong typing.
...
- Overusing `.unwrap()`/`.expect()` — causes panics and fragile error handling. This is OK in tests or examples, just don't do it in library or shipped binary code. 
...
- Put important assumptions critical to code execution in `assert!`s, especially if the variable or condition will be explicitly or implicitly evaluated later during execution. If something is important but unnecessary to check, it can be put into a `debug_assert!`. 
...
> you must follow up changesets with `cargo fmt --all -- --check` and `cargo clippy --all-targets --all-features -- -D warnings` and fix any issues they reveal.
...
> use `cargo fmt`, `cargo check`, `cargo clippy`, with `--all-features` to ensure everything is being built. 
